...
export default function AdminPage() {
  ...
  const tabs = [
    ...
    { id: "payments" as const, label: `Оплаты${payments.length > 0 ? ` (${payments.length})` : ""}` },
          ...
          <div className="space-y-4">
            ...
            <div className="border border-white/[0.04] rounded-2xl overflow-hidden">
              <table className="w-full">
                <thead><tr className="border-b border-white/[0.04]">{["Email", "План", "Метод", "Сумма", "Дата"].map((h) => (<th key={h} className="text-left text-[10px] text-zinc-600 font-medium px-4 py-3 uppercase tracking-wider">{h}</th>))}</tr></thead>
                <tbody>
                  {payments.map((p: any) => (
                    <tr key={p.id} className="border-b border-white/[0.02] hover:bg-white/[0.01] transition-colors">
                      ...
                      <td className="px-4 py-3 text-xs text-zinc-600">{p.timestamp ? new Date(p.timestamp).toLocaleString("ru-RU", { day: "2-digit", month: "2-digit", year: "2-digit", hour: "2-digit", minute: "2-digit" }) : "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              {payments.length === 0 && <div className="py-10 text-center text-xs text-zinc-700">Оплат пока нет</div>}
            </div>
          </div>
        )}
