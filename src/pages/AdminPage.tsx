        ...
        {activeTab === "settings" && (
          <div className="max-w-lg space-y-6">
            ...
            <div className="border border-white/[0.04] bg-white/[0.01] rounded-2xl p-5 space-y-4">
              ...
              <div><label className="block text-xs text-zinc-600 mb-1.5">{t('admin.globalSystemPrompt') || "Global System Prompt"}</label><textarea value={settings.globalSystemPrompt || ""} onChange={(e) => setSettings(p => ({ ...p, globalSystemPrompt: e.target.value }))} rows={3} placeholder="System prompt applied to all AI models..." className="w-full px-3.5 py-2.5 rounded-xl bg-white/[0.02] border border-white/[0.06] text-sm placeholder-zinc-700 focus:outline-none focus:border-violet-500/40 resize-none" /></div>
              <div><label className="block text-xs text-zinc-600 mb-1.5">{t('admin.freeLimit')}</label><input type="number" value={(settings as any).freeTokenLimit || 50000} onChange={(e) => setSettings(p => ({ ...p, freeTokenLimit: parseInt(e.target.value) || 0 }))} className="w-full px-3.5 py-2.5 rounded-xl bg-white/[0.02] border border-white/[0.06] text-sm focus:outline-none focus:border-violet-500/40" /></div>
              <div><label className="block text-xs text-zinc-600 mb-1.5">{t('admin.announcement')}</label><textarea value={settings.announcement} onChange={(e) => setSettings(p => ({ ...p, announcement: e.target.value }))} rows={2} placeholder="Display a text message to all users" className="w-full px-3.5 py-2.5 rounded-xl bg-white/[0.02] border border-white/[0.06] text-sm placeholder-zinc-700 focus:outline-none focus:border-violet-500/40 resize-none" /></div>
              <div><label className="block text-xs text-zinc-600 mb-1.5">{t('admin.supportEmail') || "Support Email"}</label><input type="email" value={settings.supportEmail || ""} onChange={(e) => setSettings(p => ({ ...p, supportEmail: e.target.value }))} placeholder="support@example.com" className="w-full px-3.5 py-2.5 rounded-xl bg-white/[0.02] border border-white/[0.06] text-sm focus:outline-none focus:border-violet-500/40" /></div>
            </div>
