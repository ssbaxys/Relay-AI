...
const adminModels = [
  { id: "gpt-5.4-pro", name: "GPT-5.4 Pro", provider: "OpenAI", logo: "https://img.icons8.com/fluency-systems-regular/48/chatgpt.png", logoFilter: "invert(1) brightness(2)" },
  { id: "claude-opus-4.6", name: "Claude Opus 4.6", provider: "Anthropic", logo: "https://img.icons8.com/fluency/48/claude-ai.png", logoFilter: "" },
  { id: "claude-sonnet-4.6", name: "Claude Sonnet 4.6", provider: "Anthropic", logo: "https://img.icons8.com/fluency/48/claude-ai.png", logoFilter: "" },
  { id: "gemini-3.1-pro", name: "Gemini 3.1 Pro", provider: "Google", logo: "https://img.icons8.com/skeuomorphism/32/gemini-ai.png", logoFilter: "" },
  { id: "gemini-3-flash", name: "Gemini 3 Flash", provider: "Google", logo: "https://img.icons8.com/skeuomorphism/32/gemini-ai.png", logoFilter: "" },
  { id: "mistral-large-latest", name: "Mistral Large Latest", provider: "Mistral", logo: "data:image/svg+xml;base64,", logoFilter: "" },
  { id: "deepseek-v3.2-exp", name: "DeepSeek V3.2", provider: "DeepSeek", logo: "https://img.icons8.com/color/48/deepseek.png", logoFilter: "" },
];
    ...
    const u1 = onValue(ref(db, `chats/${targetUser.uid}`), (s) => {
      ...
      if (d) {
        setChats(Object.entries(d).map(([id, v]: [string, any]) => ({
          id, title: v.title || t('chat.newChat'), model: v.model || "gpt-5.4-pro",
...
export default function AdminPage() {
  ...
  useEffect(() => { if (!authenticated) return; const u = onValue(ref(db, "users"), (s) => { const d = s.val(); if (d) { const l: UserData[] = Object.entries(d).map(([uid, val]: [string, any]) => ({ uid, displayName: val.displayName || t('admin.noName'), email: val.email || "—", plan: val.plan || "free", role: val.role || "user", lastLogin: val.lastLogin || 0, createdAt: val.createdAt || 0, banned: val.banned || false, visibleNick: val.visibleNick || "", language: val.language || "en", tokens: val.tokens || undefined })); setUsers(l); const today = new Date(); today.setHours(0, 0, 0, 0); setStats(p => ({ ...p, totalUsers: l.length, newUsersToday: l.filter(u2 => u2.createdAt >= today.getTime()).length })); } }); return () => u(); }, [authenticated]);
  ...
  useEffect(() => {
    ...
    get(ref(db, 'adminStats/totalVisits')).then(s => {
      let d = s.val();
      if (d === null) {
        get(ref(db, 'visits')).then(vs => {
          d = vs.val() ? Object.keys(vs.val()).length : 0;
          set(ref(db, 'adminStats/totalVisits'), d + 1);
        });
      } else {
        set(ref(db, 'adminStats/totalVisits'), d + 1);
      }
    });
    const u = onValue(ref(db, "adminStats/totalVisits"), (s) => {
      setStats(p => ({ ...p, totalVisits: s.val() || 0 }));
    });
    return () => u();
  }, [authenticated]);
                      ...
                      {msg.role === "user" ? (
                        ...
                        <div className="max-w-[80%]">
                          ...
                          {msg.type === "music_generating" ? (
                            <div className="flex items-center gap-3 bg-violet-600/5 border border-violet-500/10 rounded-2xl px-5 py-4 max-w-[300px]"><Music className="w-5 h-5 text-violet-400 animate-spin" style={{ animationDuration: "3s" }} /><div><p className="text-sm text-violet-400 animate-pulse">{t('admin.musicPending')}</p></div></div>
                            ...
                            <div className="w-[180px] h-[180px] rounded-2xl bg-zinc-800/50 border border-white/[0.06] flex flex-col items-center justify-center gap-3"><ImageIcon className="w-8 h-8 text-zinc-500" /><p className="text-xs text-zinc-500 animate-pulse">{t('admin.photoPending')}</p></div>
                  ...
                  <div className="max-w-2xl mx-auto space-y-3">
                    {godSearchPending && (
                      <div className="border border-blue-500/20 bg-blue-600/5 rounded-xl p-4 space-y-3">
                        <div className="flex items-center gap-2"><Search className="w-4 h-4 text-blue-400" /><span className="text-xs font-medium text-blue-400">{t('admin.searchAddSources', { count: godSearchSources.length })}</span></div>
                        <div className="flex items-center gap-2">
      ...
      <div className="max-w-6xl mx-auto px-6 py-6">
        ...
        {activeTab === "tickets" && (<div className="space-y-4"><div className="flex items-center gap-3 mb-4"><h3 className="text-sm font-medium">{t('admin.tickets')}</h3><div className="flex items-center gap-1 bg-white/[0.02] border border-white/[0.06] rounded-xl p-0.5 ml-auto">{(["all", "open", "closed"] as const).map((f) => (<button key={f} onClick={() => setTicketFilter(f)} className={`px-3 py-1.5 rounded-lg text-[11px] font-medium transition-all ${ticketFilter === f ? "bg-violet-600/10 text-violet-400" : "text-zinc-600 hover:text-zinc-400"}`}>{f === "all" ? t('admin.all') : f === "open" ? t('admin.open') : t('admin.closed')}</button>))}</div></div>
          <div className="flex gap-4" style={{ height: "calc(100vh - 220px)" }}>
            <div className="w-80 shrink-0 border border-white/[0.04] rounded-2xl overflow-y-auto">{filteredTickets.length === 0 && <div className="py-16 text-center text-xs text-zinc-700">{t('chat.noChats')}</div>}{filteredTickets.map((tk: any) => (<button key={tk.id} onClick={() => setSelectedTicket(tk)} className={`w-full flex items-center gap-3 px-4 py-3 text-left border-b border-white/[0.02] hover:bg-white/[0.01] transition-all ${selectedTicket?.id === tk.id ? "bg-violet-600/5" : ""}`}><TicketCheck className={`w-4 h-4 shrink-0 ${tk.status === "open" ? "text-emerald-400" : "text-zinc-600"}`} /><div className="flex-1 min-w-0"><p className="text-xs font-medium truncate">{tk.subject}</p><p className="text-[10px] text-zinc-600">#{tk.serialNumber} · {tk.userName}</p></div><span className={`text-[9px] font-medium px-1.5 py-0.5 rounded shrink-0 ${tk.status === "open" ? "bg-emerald-500/10 text-emerald-400" : "bg-zinc-800 text-zinc-500"}`}>{tk.status === "open" ? t('admin.open') : t('admin.closed')}</span></button>))}</div>
        ...
        {activeTab === "settings" && (
          <div className="max-w-lg space-y-6">
            <div className="border border-white/[0.04] bg-white/[0.01] rounded-2xl p-5 space-y-4">
              ...
              <div><label className="block text-xs text-zinc-600 mb-1.5">{t('admin.maintenanceEst')}</label><input value={settings.maintenanceEstimate} onChange={(e) => setSettings(p => ({ ...p, maintenanceEstimate: e.target.value }))} className="w-full px-3.5 py-2.5 rounded-xl bg-white/[0.02] border border-white/[0.06] text-sm focus:outline-none focus:border-violet-500/40" /></div>
            </div>

            <div className="border border-white/[0.04] bg-white/[0.01] rounded-2xl p-5 space-y-4">
              <h3 className="text-sm font-medium">{t('admin.newUserDefaults') || "New User Defaults"}</h3>
              <div>
                <label className="block text-xs text-zinc-600 mb-1.5">{t('admin.defaultPlan') || "Default Plan"}</label>
                <select value={(settings as any).defaultPlan || "free"} onChange={(e) => setSettings(p => ({ ...p, defaultPlan: e.target.value }))} className="w-full px-3.5 py-2.5 rounded-xl bg-white/[0.02] border border-white/[0.06] text-sm text-zinc-300 focus:outline-none focus:border-violet-500/40 cursor-pointer">
              ...
              <button onClick={() => setSettings(p => ({ ...p, registrationEnabled: !p.registrationEnabled }))} className="w-full flex items-center justify-between px-4 py-3 rounded-xl border border-white/[0.04] hover:border-violet-500/20 transition-all text-sm"><span>{t('admin.regEnabled')}</span><span className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${settings.registrationEnabled ? "bg-emerald-500/10 text-emerald-500" : "bg-red-500/10 text-red-400"}`}>{settings.registrationEnabled ? t('admin.regOpen') : t('admin.regClosed')}</span></button>
            </div>

            <div className="border border-white/[0.04] bg-white/[0.01] rounded-2xl p-5 space-y-4">
              <h3 className="text-sm font-medium">{t('admin.featureToggles') || "Feature Toggles"}</h3>
              <button onClick={() => setSettings(p => ({ ...p, enableVoiceInput: p.enableVoiceInput !== false ? false : true }))} className="w-full flex items-center justify-between px-4 py-3 rounded-xl border border-white/[0.04] hover:border-violet-500/20 transition-all text-sm">
                <span>{t('admin.enableVoiceInput') || "Enable Voice Input"}</span>
                <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${settings.enableVoiceInput !== false ? "bg-emerald-500/10 text-emerald-500" : "bg-zinc-800 text-zinc-500"}`}>{settings.enableVoiceInput !== false ? t('admin.featureOn') || "ON" : t('admin.featureOff') || "OFF"}</span>
              </button>
              <button onClick={() => setSettings(p => ({ ...p, enableFileUploads: p.enableFileUploads !== false ? false : true }))} className="w-full flex items-center justify-between px-4 py-3 rounded-xl border border-white/[0.04] hover:border-violet-500/20 transition-all text-sm">
                <span>{t('admin.enableFileUploads') || "Enable File Uploads"}</span>
                <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${settings.enableFileUploads !== false ? "bg-emerald-500/10 text-emerald-500" : "bg-zinc-800 text-zinc-500"}`}>{settings.enableFileUploads !== false ? t('admin.featureOn') || "ON" : t('admin.featureOff') || "OFF"}</span>
              </button>
            </div>

            <div className="border border-white/[0.04] bg-white/[0.01] rounded-2xl p-5 space-y-4">
              <h3 className="text-sm font-medium">{t('admin.general')} & {t('admin.systemContact') || "Contact"}</h3>
              <div><label className="block text-xs text-zinc-600 mb-1.5">{t('admin.globalSystemPrompt') || "Global System Prompt"}</label><textarea value={settings.globalSystemPrompt || ""} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setSettings(p => ({ ...p, globalSystemPrompt: e.target.value }))} rows={3} placeholder="System prompt applied to all AI models..." className="w-full px-3.5 py-2.5 rounded-xl bg-white/[0.02] border border-white/[0.06] text-sm placeholder-zinc-700 focus:outline-none focus:border-violet-500/40 resize-none" /></div>
              <div><label className="block text-xs text-zinc-600 mb-1.5">{t('admin.freeLimit')}</label><input type="number" value={(settings as any).freeTokenLimit || 50000} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSettings(p => ({ ...p, freeTokenLimit: parseInt(e.target.value) || 0 }))} className="w-full px-3.5 py-2.5 rounded-xl bg-white/[0.02] border border-white/[0.06] text-sm focus:outline-none focus:border-violet-500/40" /></div>
              <div><label className="block text-xs text-zinc-600 mb-1.5">{t('admin.announcement')}</label><textarea value={settings.announcement} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setSettings(p => ({ ...p, announcement: e.target.value }))} rows={2} placeholder="Display a text message to all users" className="w-full px-3.5 py-2.5 rounded-xl bg-white/[0.02] border border-white/[0.06] text-sm placeholder-zinc-700 focus:outline-none focus:border-violet-500/40 resize-none" /></div>
              <div><label className="block text-xs text-zinc-600 mb-1.5">{t('admin.supportEmail') || "Support Email"}</label><input type="email" value={settings.supportEmail || ""} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSettings(p => ({ ...p, supportEmail: e.target.value }))} placeholder="support@example.com" className="w-full px-3.5 py-2.5 rounded-xl bg-white/[0.02] border border-white/[0.06] text-sm focus:outline-none focus:border-violet-500/40" /></div>
            </div>

            <button onClick={saveSettings} className={`flex items-center justify-center gap-2 w-full py-3 rounded-xl text-sm font-medium transition-all ${saved ? "bg-emerald-600 text-white" : "bg-violet-600 text-white hover:bg-violet-500"}`}>{saved ? <><Check className="w-4 h-4" /> {t('admin.saved')}</> : <><Save className="w-4 h-4" /> {t('admin.save')}</>}</button>
          </div>
        )}
